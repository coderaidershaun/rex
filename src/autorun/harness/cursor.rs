//! Cursor Agent CLI: process spawning and output parsing.
...
/// Spawn `agent -p` (Cursor CLI) with process group isolation.
pub fn spawn_agent(
    project_dir: &Path,
    prompt: &str,
    system_prompt: &str,
    ...
    // Cursor has no --append-system-prompt, so prepend to the prompt text.
    ...
    // Cursor does not support --name, but use session_name for logging.
    let _ = session_name;
    ...
    info!("spawning agent process (cursor)");
    let child = cmd
        .spawn()
    ...
    let pgid = pid as i32;
    info!(pid, pgid, "agent process started (cursor)");
